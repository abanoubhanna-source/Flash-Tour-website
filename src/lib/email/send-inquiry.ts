...
const INQUIRY_RECIPIENT = "info@flashtour.travel";

export async function sendInquiryEmail(input: InquiryEmailInput): Promise<{ ok: boolean; message: string }> {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    return { ok: false, message: "Email sending isn't configured yet. Please contact us directly at info@flashtour.travel." };
  }
