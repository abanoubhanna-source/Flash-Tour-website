...
export const contentSchema = z.object({
  ...
  categoryId: z.uuid().nullable().default(null), roomsOrCabins: z.number().int().nonnegative().nullable().default(null), facilities: z.array(z.string().trim().min(1).max(160)).max(50).default([]), diningOptions: z.array(z.string().trim().min(1).max(160)).max(50).default([]), accessibility: z.array(z.string().trim().min(1).max(160)).max(50).default([]), gallery: z.array(imageSchema).max(40).default([]), displayOrder: z.number().int().min(0).max(99999).default(0), isActive: z.boolean().default(true),
