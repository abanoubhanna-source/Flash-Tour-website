...
export async function getCollectionEditor(id: string, expectedType?: ManagedContentType): Promise<CmsCollectionEditor | null> {
  ...
  let parentCountryById = new Map<string, string>();
  if (entry.content_type === "destination_attraction" && possibleParents?.length) {
    const placeIds = possibleParents.map((p) => p.id);
    const { data: placeRelations } = await s.from("content_relations").select("source_id,target_id").eq("relation_type", "contains_place").in("target_id", placeIds);
    const countryIds = [...new Set((placeRelations ?? []).map((r) => r.source_id))];
    const { data: countries } = countryIds.length ? await s.from("content_entries").select("id,title").in("id", countryIds) : { data: [] as { id: string; title: string }[] };
    const countryTitleById = new Map((countries ?? []).map((c) => [c.id, c.title]));
    parentCountryById = new Map((placeRelations ?? []).map((r) => [r.target_id, countryTitleById.get(r.source_id) ?? ""]));
  }
