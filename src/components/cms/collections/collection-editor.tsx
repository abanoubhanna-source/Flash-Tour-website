...
function ContentTab({ entry, draft, updateContent, updateDraft, updateArray }: { entry: CmsCollectionEditor; draft: ManagedDraft; updateContent: <Key extends keyof ManagedContent>(key: Key, value: ManagedContent[Key]) => void; updateDraft: Dispatch<SetStateAction<ManagedDraft>>; updateArray: (key: ArrayField, values: string[]) => void }) {
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2"><Field label="Rooms / cabins" type="number" value={draft.content.roomsOrCabins?.toString() ?? ""} onChange={(value) => updateContent("roomsOrCabins", value === "" ? null : Number(value))} /><Field label="Display order" type="number" value={String(draft.content.displayOrder)} onChange={(value) => updateContent("displayOrder", Number(value) || 0)} /></div>
    ...
    {(["facilities", "diningOptions", "accessibility"] as const).map((key) => <ArrayEditor key={key} label={key === "diningOptions" ? "Dining options" : key[0].toUpperCase() + key.slice(1)} values={draft.content[key]} onChange={(values) => updateArray(key, values)} />)}
