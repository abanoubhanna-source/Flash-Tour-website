...
export default async function UsersModulePage() {
  ...
  const [{ data: profiles, error: profilesError }, { data: roles, error: rolesError }] = await Promise.all([
    supabase.from("profiles").select("id,display_name,status,user_roles!user_roles_user_id_fkey(role_id,roles!user_roles_role_id_fkey(name,key,rank))").order("created_at"),
