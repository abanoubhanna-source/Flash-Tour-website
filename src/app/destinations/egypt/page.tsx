...
export default function EgyptComprehensivePage() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center w-full bg-white overflow-hidden">
      ...
      {/* =========================================
          PART 1: HOSPITALITY (From previous code)
      ========================================= */}
      <div id="hospitality" className="w-full pt-16">
        
        {/* The Royal Fleet (Bento Grid Layout) */}
        <section className="w-full py-24 bg-slate-50 relative z-20">
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                <div className="flex items-center gap-3 mb-4">
                  <Ship className="w-6 h-6 text-[#157670]" />
                  <h2 className="text-4xl font-bold text-[#0F162A] font-en tracking-tight">The Royal Fleet</h2>
                </div>
                <p className="text-lg text-slate-500 font-en max-w-xl">
                  Commanding the historic waters of the Nile with an unparalleled fleet of luxury vessels.
                </p>
              </motion.div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {cruises.map((cruise, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className={`group relative h-[450px] rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 ${cruise.cols}`}>
                  <Image src={cruise.image} alt={cruise.name} fill className="object-cover group-hover:scale-105 transition-transform duration-[1.5s] ease-out" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0F162A] via-[#0F162A]/40 to-transparent"></div>
                  <div className="absolute inset-0 p-8 flex flex-col justify-end text-white z-10">
                    <span className="text-[#F1B820] font-bold text-xs uppercase tracking-widest font-en mb-2 block">{cruise.type}</span>
                    <h3 className="text-3xl font-bold font-en mb-3">{cruise.name}</h3>
                    <p className="text-slate-300 font-en text-sm leading-relaxed opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 line-clamp-2">{cruise.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
            <div className="text-center mb-20">
              <div className="flex items-center justify-center gap-3 mb-4">
                <Building2 className="w-8 h-8 text-[#F1B820]" />
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-[#0F162A] font-en tracking-tight mb-4">Exclusive <span className="text-[#F1B820]">Sanctuaries</span></h2>
              <p className="text-slate-500 font-en text-lg max-w-2xl mx-auto">From the pristine Red Sea coast to the historic heart of Cairo.</p>
            </div>
              ...
              {hotels.map((hotel, idx) => {
                ...
                return (
                  <div key={idx} className={`flex flex-col lg:flex-row items-center gap-8 ${isEven ? '' : 'lg:flex-row-reverse'}`}>
                    <motion.div initial={{ opacity: 0, x: isEven ? -40 : 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full lg:w-3/5 h-[500px] relative rounded-[3rem] overflow-hidden shadow-2xl group">
                      <Image src={hotel.image} alt={hotel.name} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                    </motion.div>

                    <motion.div initial={{ opacity: 0, x: isEven ? 40 : -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full lg:w-2/5 p-10 md:p-14 bg-slate-50 rounded-[3rem] border border-slate-100 hover:bg-white hover:shadow-xl transition-all duration-500 z-10">
                      <h3 className="text-4xl font-bold text-[#0F162A] font-en mb-4">{hotel.name}</h3>
                      <p className="text-[#157670] font-bold uppercase tracking-widest text-sm font-en mb-6 flex items-center gap-2">
                        <Anchor className="w-4 h-4" /> {hotel.location}
                      </p>
                      <p className="text-lg text-slate-600 font-en leading-relaxed mb-8">{hotel.desc}</p>
                      <button className="flex items-center gap-2 text-[#0F162A] font-bold font-en hover:text-[#157670] transition-colors group">
                        Explore Property <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                      </button>
                    </motion.div>
                  </div>
                );
        ...
        {/* Culinary & Boats */}
        <section className="w-full py-24 bg-[#0F162A] text-white">
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              {/* Fine Dining */}
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <div className="flex items-center gap-3 mb-8">
                  <UtensilsCrossed className="w-8 h-8 text-[#F1B820]" />
                  <h2 className="text-4xl font-bold font-en tracking-tight">Culinary <span className="text-[#F1B820]">Excellence</span></h2>
                </div>
                <div className="space-y-6">
                  {dining.map((rest, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 p-8 rounded-3xl hover:bg-white/10 transition-colors group">
                      <h3 className="text-2xl font-bold text-white font-en mb-2">{rest.name}</h3>
                      <p className="text-[#157670] font-bold text-xs uppercase tracking-widest font-en mb-4">{rest.type}</p>
                      <p className="text-slate-300 font-en leading-relaxed text-sm">{rest.desc}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                <div className="flex items-center gap-3 mb-8">
                  <Waves className="w-8 h-8 text-[#157670]" />
                  <h2 className="text-4xl font-bold font-en tracking-tight">Red Sea <span className="text-[#157670]">Command</span></h2>
                </div>
                <div className="relative h-[350px] w-full rounded-[2.5rem] overflow-hidden shadow-2xl group">
                  <Image src="/images/flash-boats.jpg" alt="Flash Yachting" fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0F162A] via-[#0F162A]/20 to-transparent"></div>
                  <div className="absolute bottom-8 left-8 pr-8">
                    <h3 className="text-2xl font-bold text-white font-en mb-2">Flash 3, 4 & 5 Yachts</h3>
                    <p className="text-slate-300 font-en text-sm leading-relaxed">
                      Command the Red Sea with our exclusive luxury speed boats. Perfect for private island hopping and exclusive marine excursions tailored to your desires.
                    </p>
                  </div>
                </div>
              </motion.div>
